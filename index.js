...
app.post('/reiniciar-bot', verifyToken, (req, res) => {
    ...
    exec('pgrep -f "^node .$"', (error, stdout, stderr) => {
        ...
        exec(`kill ${stdout}`, (errorKill, stdoutKill, stderrKill) => {
            ...
            //reinicio
            exec('nohup npm --prefix /home/ubuntu/tbot/ start > /home/ubuntu/tbot/nohup.out 2>&1 < /dev/null &', (errorStart, stdoutStart, stderrStart) => {
                if (errorStart) {
...
app.post('/encender-bot', verifyToken, (req, res) => {
    ...
    exec('pgrep -f "^node .$"', (error, stdout, stderr) => {
        ...
        if (error) {
            return exec('nohup npm --prefix /home/ubuntu/tbot/ start > /home/ubuntu/tbot/nohup.out 2>&1 < /dev/null &', (errorStart, stdoutStart, stderrStart) => {
                if (errorStart) 
                {
